...
const BASE_NODES: Record<PageKey, VisualNode[]> = {
  ...
  'portal-vagas': [
    { id: 'portal-header', kind: 'card', name: 'Cabeçalho do portal', text: 'RH TRANSFORMA', x: 0, y: 0, width: 1180, height: 82, background: '#ffffff', color: '#123657', fontSize: 18, radius: 0, visible: true },
    { id: 'portal-logo', kind: 'text', name: 'Nome / logo', text: 'RH TRANSFORMA', x: 42, y: 18, width: 210, height: 48, background: 'transparent', color: '#123657', fontSize: 22, radius: 0, visible: true },
  ...
  'acesso-master': [
    { id: 'title', kind: 'text', name: 'Título principal', text: 'Painel Master RH TRANSFORMA', x: 56, y: 44, width: 500, height: 52, background: 'transparent', color: '#0f172a', fontSize: 32, radius: 0, visible: true },
    ...
    <div className="-m-1 flex h-[calc(100dvh-7.5rem)] min-h-[680px] flex-col overflow-hidden rounded-2xl border border-slate-800 bg-[#0a1220] shadow-2xl lg:-m-3">
      <header className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 bg-slate-950 px-3 py-2.5">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0"><h2 className="truncate text-sm font-black text-white">Editor Visual RH TRANSFORMA</h2><p className="text-[10px] text-slate-500">Edite o layout sem mexer no código</p></div>
        </div>
...
function VisualElement({ node, selected, zIndex, mode, onPointerDown }: { node: VisualNode; selected: boolean; zIndex: number; mode: EditorMode; onPointerDown: (event: React.PointerEvent, node: VisualNode) => void }) {
  ...
  return <div {...shared} className={`flex flex-col justify-between border border-slate-200 p-5 font-bold shadow-sm ${mode === 'edit' ? 'cursor-move' : ''} ${selection}`}><span>{node.text}</span><span className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Componente RH TRANSFORMA</span></div>;
}
