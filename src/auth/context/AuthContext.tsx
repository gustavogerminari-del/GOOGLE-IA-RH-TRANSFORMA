...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [globalDisabledModules, setGlobalDisabledModules] = useState<Record<string, boolean>>({});
  ...
  // platform_modules_realtime_v1
  useEffect(() => {
    let disposed = false;
    let unsubscribe = () => {};
    void import('../../firebase/firestore').then(({ collection, onSnapshot }) => {
      if (disposed) return;
      unsubscribe = onSnapshot(collection(db, 'modulos'), (snapshot) => {
        const next: Record<string, boolean> = {};
        snapshot.docs.forEach((item) => {
          const data = item.data() as Record<string, any>;
          const key = String(data.key || data.id || item.id || '').trim();
          if (key && data.ativo === false) next[key] = true;
        });
        setGlobalDisabledModules(next);
      }, (error) => console.warn('[AuthContext] Falha ao observar catálogo global de módulos.', error));
    }).catch((error) => console.warn('[AuthContext] Firestore realtime indisponível para módulos globais.', error));
  ...
  const isGloballyDisabledModule = (moduleKey: string) => {
    const raw = String(moduleKey || '').trim();
    const alias: Record<string, string> = {
      candidatos: 'recrutamento',
      contratacoes: 'recrutamento',
      'banco-talentos': 'bancoTalentos',
      siteVagas: 'siteVagasPersonalizado',
      'site-vagas': 'siteVagasPersonalizado',
      'equipe-interna': 'equipeInterna',
      'departamento-pessoal': 'departamentoPessoal',
      dp: 'departamentoPessoal',
      colaboradores: 'departamentoPessoal',
      admissoes: 'departamentoPessoal',
      'ponto-digital': 'ponto',
      jornada: 'ponto',
      'folha-pagamento': 'folha',
      'ferias-beneficios': 'feriasBeneficios',
      ferias: 'feriasBeneficios',
      beneficios: 'feriasBeneficios',
      documentos: 'documentosAssinatura',
      auditoria: 'auditoriaLogs',
      relatorios: 'relatoriosAvancados',
      'implantacao-migracao': 'implantacaoMigracao',
      migracao: 'implantacaoMigracao',
    };
    const canonical = raw.startsWith('headhunter-') ? 'headhunter' : (alias[raw] || raw);
    return globalDisabledModules[canonical] === true;
  };

  const isModuleActive = (moduleKey: string) => {
    if (!user) return false;
    if (isGloballyDisabledModule(moduleKey)) return false;
  ...
  const hasScreenAccess = (screenKey: ScreenRouteKey) => {
    if (!user) return false;
    if (screenKey === 'acesso-master' || screenKey.startsWith('master-')) {
