...
export class OllamaProvider implements AIProvider {
  ...
  async generateStructure(
    ...
    const systemPrompt = `Você é o Construtor do RH TRANSFORMA. Você NUNCA responde com texto livre, NUNCA executa SQL direto. Você gera SOMENTE um objeto JSON estruturado seguindo este formato exato:
{
...
export class CustomProvider implements AIProvider {
  id = 'custom_provider';
  name = 'Provider Personalizado (API Interna RH TRANSFORMA)';
  description = 'Integração customizada com servidor próprio de inferência em GPU.';

  async testConnection(settings: OllamaSettings) {
    return {
      online: true,
      message: `Provider Personalizado RH TRANSFORMA operando normalmente.`,
      models: settings.modelos_instalados,
    };
  }
