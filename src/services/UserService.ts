...
async function createUserWithSecondaryFirebaseApp(input: {
  ...
  const appName = `rhtransforma-provision-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  ...
  try {
    ...
    // Se o e-mail já existia no Authentication, nunca reaproveitamos a conta
    // cegamente para outra empresa/perfil. A senha correta prova posse da conta,
    // mas não autoriza trocar o tenant nem rebaixar/promover uma identidade de
    // plataforma. Isso evita sequestro acidental de acesso ao recriar empresas.
    if (!createdNewAuthUser) {
      const official = await getDoc(doc(db, 'usuarios', createdUser.uid));
      const compat = official.exists() ? null : await getDoc(doc(db, 'users', createdUser.uid));
      const existing = official.exists() ? official.data() : (compat?.exists() ? compat.data() : null);
      if (existing) {
        const existingRole = normalizedAccessRole(existing.role || existing.perfil);
        const existingType = normalizedAccessRole(existing.tipoUsuario);
        const existingIsPlatform = ['MASTER', 'MASTER_ADMIN', 'DEVELOPER_ADMIN', 'DEVELOPER', 'DESENVOLVEDOR'].includes(existingRole)
          || ['MASTER', 'MASTER_ADMIN', 'DEVELOPER_ADMIN', 'DEVELOPER', 'DESENVOLVEDOR'].includes(existingType);
        const requestedRole = normalizedAccessRole(input.role);
        const requestedIsPlatform = ['MASTER', 'MASTER_ADMIN', 'DEVELOPER_ADMIN', 'DEVELOPER', 'DESENVOLVEDOR'].includes(requestedRole);
        const existingCompanyId = String(existing.empresaId || existing.companyId || '').trim();

        if (existingIsPlatform !== requestedIsPlatform) {
          throw new Error('Este e-mail já pertence a um acesso protegido de outro tipo e não pode ser reutilizado neste cadastro.');
        }
        if (!requestedIsPlatform && existingCompanyId && existingCompanyId !== input.companyId) {
          throw new Error('Este e-mail já está vinculado a outra empresa e não pode ser reaproveitado. Use outro e-mail ou remova corretamente o acesso anterior.');
        }
        if (requestedIsPlatform && existingRole && existingRole !== requestedRole) {
          throw new Error('Este e-mail já pertence a uma identidade protegida diferente e não pode ter o perfil alterado por este fluxo.');
        }
      }
    }
...
export class UserService {
  static async create(userData: Partial<UserProfile> & { password?: string; allowClientSideFallback?: boolean }): Promise<UserProfile> {
    ...
    let uid: string;
    try {
      const resData = await authorizedRequest('/api/auth/users', {
        method: 'POST',
        body: JSON.stringify({
          email,
      ...
      const serverUid = String(resData.uid || resData.user?.uid || '').trim();
      if (!serverUid) throw new Error('A API não retornou o UID do usuário criado.');
      uid = serverUid;
    } catch (err: any) {
    ...
    const persistedProfile = {
      ...
      nome: displayName,
      perfil: role,
      tipoUsuario: isMaster ? 'MASTER' : isDeveloper ? 'DEVELOPER' : (userData.tipoUsuario || 'EMPRESA'),
      ativo: isAtivo,
      empresaId,
      companyId: empresaId,
      companyName,
      permissions: provisionedPermissions,
      permissoes: provisionedPermissions,
      modules: userData.modules || {},
    ...
    profileBatch.set(doc(db, 'usuarios', finalUid), persistedProfile, { merge: true });
    profileBatch.set(doc(db, 'users', finalUid), persistedProfile, { merge: true });
    await profileBatch.commit();

    const confirmedProfile = await getDoc(doc(db, 'usuarios', finalUid));
    if (!confirmedProfile.exists()) {
      throw new Error('O acesso foi criado no Authentication, mas o perfil não apareceu em Usuários e Permissões.');
    }
    const confirmedCompanyId = String(confirmedProfile.data()?.empresaId || confirmedProfile.data()?.companyId || '').trim();
    if (!isPlatformUser && confirmedCompanyId !== companyId) {
      throw new Error('O acesso foi criado, mas não ficou vinculado à empresa correta.');
    }

    // RH_PONTO_USER_PROVISION_V3
    const pontoAccess = String((userData as any).pontoAccess || '').toUpperCase();
    if (!isPlatformUser && companyId && ['TOTAL', 'PONTO_ESPELHO'].includes(pontoAccess)) {
      const sessionToken = await auth.currentUser?.getIdToken();
      if (!sessionToken) throw new Error('Sessão indisponível para provisionar o acesso no sistema de Ponto.');
      const pontoResponse = await fetch('/api/ponto/provision-user', {
        method: 'POST',
        headers: { 'content-type': 'application/json', authorization: 'Bearer ' + sessionToken },
        body: JSON.stringify({
          companyId, name: displayName, email: String(userData.email || '').trim().toLowerCase(),
          access: pontoAccess, externalEmployeeId: String((userData as any).externalEmployeeId || '').trim() || null,
        }),
      });
      const pontoUser = await pontoResponse.json().catch(() => ({}));
      if (!pontoResponse.ok || !pontoUser?.ok) {
        throw new Error('O acesso foi criado no RH-MIL, mas o Ponto não foi liberado: ' + (pontoUser?.message || 'falha no provisionamento.'));
      }
    }
  ...
  static async update(uid: string, data: Partial<UserProfile>): Promise<void> {
    ...
    try {
      await authorizedRequest(`/api/auth/users/${encodeURIComponent(uid)}`, {
        method: 'PATCH',
  ...
  static async delete(uid: string): Promise<void> {
    const current = await this.getById(uid);
    if (!current) throw new Error('Usuário não encontrado.');
    if (uid === auth.currentUser?.uid || isProtectedMasterProfile(current)) {
      throw new Error('O acesso MASTER é protegido e não pode ser excluído.');
    }

    let authDeletionPending = false;
    try {
      await authorizedRequest(`/api/auth/users/${encodeURIComponent(uid)}`, { method: 'DELETE' });
    } catch (requestError: any) {
      if (!canUseClientFirebaseFallback(requestError, [404, 405, 501])) throw requestError;

      // No Worker estático a API Admin pode não existir. Remover os dois perfis
      // do Firestore revoga imediatamente o acesso porque toda sessão operacional
      // exige perfil ativo em usuarios/users. A conta Auth órfã pode ser limpa
      // posteriormente pelo backend administrativo sem risco de acesso.
      const batch = writeBatch(db);
    ...
    await safeAudit({
      action: 'DELETE',
      description: authDeletionPending
        ? `Acesso ${uid} removido do Firestore; conta Firebase Authentication aguardando limpeza administrativa`
        : `Usuário ${uid} excluído do Firebase`,
      moduleName: 'Configurações',
      targetEntity: 'Usuário'
    }, 'Exclusão do usuário');
  }
  ...
  static async list(companyId?: string): Promise<UserProfile[]> {
    ...
    const readCollection = async (collectionName: string) => {
      const snapshots = companyId
        ? await Promise.all([
            getDocs(query(collection(db, collectionName), where('empresaId', '==', companyId))),
            getDocs(query(collection(db, collectionName), where('companyId', '==', companyId))),
          ])
        : [await getDocs(collection(db, collectionName))];
      for (const snap of snapshots) {
        snap.forEach((d) => byUid.set(d.id, normalizeProfile(d.id, d.data() as RawUserProfile)));
      }
    };

    let officialError: unknown = null;
    try {
      await readCollection(COLLECTION_NAME);
    } catch (err) {
      officialError = err;
      console.warn('Erro ao listar usuarios; tentando espelho users:', err);
    }

    try {
      await readCollection('users');
    } catch (legacyError) {
      if (officialError) console.warn('Erro também no espelho users:', legacyError);
    }
