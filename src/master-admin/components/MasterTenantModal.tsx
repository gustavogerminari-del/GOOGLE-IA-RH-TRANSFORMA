...
const MODULE_LABELS: Array<[keyof TenantModulePermissions, string]> = [
  ...
  ['implantacaoMigracao', 'Implantação e Migração'],
  ['folha', 'Folha'],
  ['ponto', 'Ponto'],
];
...
export function MasterTenantModal({ tenant, onClose, onSave }: MasterTenantModalProps) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!isEditing) {
      ...
      if (adminPassword.length < 8) return setError('A senha inicial deve ter pelo menos 8 caracteres.');
    ...
    try {
      const payload: TenantSaveInput = isEditing
        ? { ...form, mode: 'edit' }
        : { ...form, adminCredentials: { ...(form.adminCredentials || {}), adminEmail: form.ownerEmail.trim().toLowerCase() }, mode: 'create', adminPassword, confirmAdminPassword, sendCredentialsEmail };
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 p-2 sm:p-4" role="dialog" aria-modal="true" aria-label={title}>
      <form onSubmit={submit} className="flex max-h-[95vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 text-slate-100 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-800 px-4 py-4 sm:px-6">
          <div><h2 className="text-lg font-black text-white">{title}</h2><p className="mt-1 text-xs text-slate-400">O login é exclusivo do RH-MIL. O PRONTO-RH é acessado por SSO e nunca recebe a senha deste usuário.</p></div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
          ...
          {activeTab === 'empresa' && <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="E-mail de contato"><input type="email" className={baseInput} value={form.ownerEmail} onChange={(e) => { const email = e.target.value; setForm((current) => ({ ...current, ownerEmail: email, adminCredentials: isEditing ? current.adminCredentials : { ...(current.adminCredentials || {}), adminEmail: email } })); }} /></Field>
          ...
          {activeTab === 'administrador' && <div className="space-y-4">
            {isEditing ? (
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="E-mail do administrador (automático)"><input type="email" className={baseInput} value={form.ownerEmail} readOnly aria-readonly="true" title="Este e-mail é puxado automaticamente do E-mail de contato da empresa." /></Field>
                <div />
                <Field label="Senha Inicial"><input type="password" className={baseInput} value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} minLength={8} required={!isEditing} autoComplete="new-password" /></Field>
                <Field label="Confirmar Senha"><input type="password" className={baseInput} value={confirmAdminPassword} onChange={(e) => setConfirmAdminPassword(e.target.value)} minLength={8} required={!isEditing} autoComplete="new-password" /></Field>
